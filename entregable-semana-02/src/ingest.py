...
def load_corpus() -> list[dict]:
    """Carga los 4 corpora y devuelve lista de chunks listos para embedding."""
    chunks: list[dict] = []

    productos = json.loads((CORPUS_DIR / "productos.json").read_text(encoding="utf-8"))
    chunks.extend(chunk_json_records(productos, "productos.json", "producto"))

    vets = json.loads((CORPUS_DIR / "veterinarias.json").read_text(encoding="utf-8"))
    chunks.extend(chunk_json_records(vets, "veterinarias.json", "veterinaria"))
